...
//! through no mapping here. `F` does not ctrl+click: measured, a ctrl+click
//! makes a target with no `openerId`, which the tab list does not adopt, so
//! the person would see nothing happen. It opens the link's href through this
//! program's own `Target.createTarget` instead.
//!
