...
pub struct Options {
    ...
    pub force_dark: bool,
    /// Start in normal mode rather than insert: the letters are the
    /// program's keys from the first one. See [`crate::normal`].
...
fn hint_key(
    ...
    match step {
        ...
        hints::Typed::Chosen(hint) => {
            ...
            cancel_hints(tabs, chrome);
            follow_hint(pane, tabs, browser, chrome, hint, new_tab)?;
        }
    }
    redraw_row(pane, tabs, chrome)?;
    Ok(true)
...
///
/// The new tab is this program's own `Target.createTarget`, [`open_tab`],
/// and never a modified click: a ctrl+click makes a target with no opener,
/// which [`crate::tabs`] does not adopt (see [`crate::hints`]). It comes to
/// the front, as `ctrl+t`'s does; a tab opened behind is one line here.
///
...
fn follow_hint(
    ...
    if new_tab && hint.kind == hints::Kind::Link && !hint.href.is_empty() {
        let was = tabs.active_target().map(str::to_string);
        let appearance = chrome.appearance;
        match open_tab(tabs, browser, &appearance, &hint.href) {
            Ok(_) => switched(pane, tabs, browser, chrome, was)?,
...
mod tests {
    ...
    fn escape_goes_to_whatever_has_the_row_then_the_hints_then_the_load_then_the_page() {
        ...
        // Every combination of the four things that can have the row, hints
        ...
        for mask in 0..64u32 {
            let [open_bar, open_find, dialog, path, hinting, loading] =
            ...
            assert_eq!(
                ...
                "bar {open_bar}, find {open_find}, dialog {dialog}, path {path}, \
                 hinting {hinting}, loading {loading}"
            );
        }
